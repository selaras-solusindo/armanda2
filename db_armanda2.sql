...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `db_armanda2`.`v_invoice_pelaksanaan` AS select `db_armanda2`.`t_invoice_pelaksanaan`.`invoice_id` AS `invoice_id`,group_concat(date_format(`db_armanda2`.`t_invoice_pelaksanaan`.`tanggal`,'%d-%m-%Y') separator ', ') AS `tgl_pelaksanaan` from `db_armanda2`.`t_invoice_pelaksanaan` group by `db_armanda2`.`t_invoice_pelaksanaan`.`invoice_id`;
...
(1, '23-01-2017, 24-01-2017'),
(2, '01-02-2017, 02-02-2017, 03-02-2017');
