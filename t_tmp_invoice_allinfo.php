...
$t_tmp_invoice_all = NULL;

//
// Table class for t_tmp_invoice_all
//
class ct_tmp_invoice_all extends cTable {
	var $temp_id;
	var $tanggal;
	var $nama;
	var $no_kwitansi;
	...
	function __construct() {
		...
		$this->TableVar = 't_tmp_invoice_all';
		$this->TableName = 't_tmp_invoice_all';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_tmp_invoice_all`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// temp_id
		$this->temp_id = new cField('t_tmp_invoice_all', 't_tmp_invoice_all', 'x_temp_id', 'temp_id', '`temp_id`', '`temp_id`', 3, -1, FALSE, '`temp_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// tanggal
		$this->tanggal = new cField('t_tmp_invoice_all', 't_tmp_invoice_all', 'x_tanggal', 'tanggal', '`tanggal`', ew_CastDateFieldForLike('`tanggal`', 0, "DB"), 133, 0, FALSE, '`tanggal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->tanggal->Sortable = TRUE; // Allow sort
		$this->tanggal->FldDefaultErrMsg = str_replace("%s", $GLOBALS["EW_DATE_FORMAT"], $Language->Phrase("IncorrectDate"));
		$this->fields['tanggal'] = &$this->tanggal;

		// nama
		$this->nama = new cField('t_tmp_invoice_all', 't_tmp_invoice_all', 'x_nama', 'nama', '`nama`', '`nama`', 200, -1, FALSE, '`nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama->Sortable = TRUE; // Allow sort
		$this->fields['nama'] = &$this->nama;

		// no_kwitansi
		$this->no_kwitansi = new cField('t_tmp_invoice_all', 't_tmp_invoice_all', 'x_no_kwitansi', 'no_kwitansi', '`no_kwitansi`', '`no_kwitansi`', 200, -1, FALSE, '`no_kwitansi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->no_kwitansi->Sortable = TRUE; // Allow sort
		$this->fields['no_kwitansi'] = &$this->no_kwitansi;

		// nomor
		$this->nomor = new cField('t_tmp_invoice_all', 't_tmp_invoice_all', 'x_nomor', 'nomor', '`nomor`', '`nomor`', 200, -1, FALSE, '`nomor`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nomor->Sortable = TRUE; // Allow sort
		$this->fields['nomor'] = &$this->nomor;

		// no_sertifikat
		$this->no_sertifikat = new cField('t_tmp_invoice_all', 't_tmp_invoice_all', 'x_no_sertifikat', 'no_sertifikat', '`no_sertifikat`', '`no_sertifikat`', 201, -1, FALSE, '`no_sertifikat`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->no_sertifikat->Sortable = TRUE; // Allow sort
		$this->fields['no_sertifikat'] = &$this->no_sertifikat;

		// tgl_pelaksanaan
		$this->tgl_pelaksanaan = new cField('t_tmp_invoice_all', 't_tmp_invoice_all', 'x_tgl_pelaksanaan', 'tgl_pelaksanaan', '`tgl_pelaksanaan`', ew_CastDateFieldForLike('`tgl_pelaksanaan`', 0, "DB"), 133, 0, FALSE, '`tgl_pelaksanaan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// total_ppn
		$this->total_ppn = new cField('t_tmp_invoice_all', 't_tmp_invoice_all', 'x_total_ppn', 'total_ppn', '`total_ppn`', '`total_ppn`', 4, -1, FALSE, '`total_ppn`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('temp_id', $rs))
				ew_AddFilter($where, ew_QuotedName('temp_id', $this->DBID) . '=' . ew_QuotedValue($rs['temp_id'], $this->temp_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->temp_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@temp_id@", ew_AdjustSql($this->temp_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "t_tmp_invoice_alllist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_tmp_invoice_allview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_tmp_invoice_allview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_tmp_invoice_alladd.php?" . $this->UrlParm($parm);
		else
			$url = "t_tmp_invoice_alladd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t_tmp_invoice_alledit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t_tmp_invoice_alladd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_tmp_invoice_alldelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "temp_id:" . ew_VarToJson($this->temp_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->temp_id->CurrentValue)) {
			$sUrl .= "temp_id=" . urlencode($this->temp_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["temp_id"]))
				$arKeys[] = ew_StripSlashes($_POST["temp_id"]);
			elseif (isset($_GET["temp_id"]))
				$arKeys[] = ew_StripSlashes($_GET["temp_id"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->tgl_pelaksanaan->EditValue = ew_FormatDateTime($this->tgl_pelaksanaan->CurrentValue, 8);
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->temp_id->Exportable) $Doc->ExportCaption($this->temp_id);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->temp_id->Exportable) $Doc->ExportField($this->temp_id);
